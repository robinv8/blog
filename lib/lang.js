export const lang = {
  zh: {
    NAV: {
      INDEX: '首页',
      SEARCH: '搜索',
      ABOUT: '关于',
      FRINEDS: '友链',
      PROJECTS: '作品',
      WRITING: '写作',
      NOTES: '笔记',
      NEWSLETTER: '周刊',
      BOOKS: '书架',
      CONTACT: '联系'
    },
    PAGINATION: {
      PREV: '上一页',
      NEXT: '下一页'
    },
    LAYOUT: {
      NOTICE_TEXT: '对于本文内容有任何疑问, 可与我联系.',
      PAY_BUTTON: '微信赞赏',
      NOTICE_BUTTON: '联系我',
      COMMENT_TITLE: '评论区'
    },
    SEARCH: {
      ONLY_SEARCH: '只搜索标签',
      PLACEHOLDER: '搜索文章',
      NOT_FOUND: '没有找到有关文章'
    },
    HERO: {
      HOME: {
        CONTACT_BUTTON: '联系合作',
        CONTACT_BUTTON_DES: '项目 / 咨询 / 交流',
        RSS_BUTTON: '订阅更新',
        PROJECTS_BUTTON: '查看作品',
        PROJECTS_BUTTON_DES: '开源与产品',
        GREETING: '你好，我是'
      },
      NEWSLETTER: {
        SUBSCRIPTION_HEAD: '订阅渠道',
        TG_CHANNEL: 'Telegram 频道',
        RSS_BUTTON: '订阅 Newsletter'
      },
      NOTES: {
        TEXT_FOOTER: '本页面根据 Craft.do 文档自动生成'
      },
      RSS_BUTTON_DES: '任意 RSS 阅读器',
      RSS_BUTTON_COPIED: '已复制',
      RSS_BUTTON_DES_COPIED: '已复制 RSS 链接'
    },
    HOME: {
      PROJECTS_TITLE: '精选作品',
      PROJECTS_MORE: '查看全部作品',
      POSTS_TITLE: '最新写作',
      WORK_TITLE: '作品',
      WORK_MORE: '全部作品 →',
      WRITING_TITLE: '写作',
      WRITING_MORE: '全部文章 →',
      WRITING_SUB: '构建过程与思考。'
    },
    PROJECTS: {
      TITLE: '作品',
      DESCRIPTION: '产品在建、开源贡献、实验项目——分级列出，不拍平。',
      VISIT: '访问',
      FEATURED: '精选',
      ALL: '全部',
      TIER: {
        product: '产品',
        oss: '开源',
        lab: '实验'
      },
      TIER_INTRO: {
        product: '正在交付或已上线的产品与工具',
        oss: '在大项目里留下的贡献与身份',
        lab: '探索中的实验，未必成为产品'
      }
    },
    CONTACT: {
      TITLE: '联系我',
      DESCRIPTION: '合作、开源协作或随便聊聊，欢迎留言。',
      TG_DESCRIPTION: '也可直接联系 Telegram: ',
      SUCCESS_MESSAGE: '消息已发送, 我会尽快回复到你的邮箱.',
      FORM_USERNAME: '你的昵称',
      FORM_EMAIL: '你的邮箱*',
      FORM_CONTENT: '消息内容',
      SEND_BUTTON: '发送',
      FORM_EMAIL_REQUIRED: '*可以填写其他有效联系方式',
      FAILED_MESSAGE: '抱歉, 发送失败, 请直接联系我的 Telegram 吧.'
    },
    FOOTER: {
      COPYRIGHT_START: '本站原创内容基于 ',
      COPYRIGHT_NAME: 'CC BY-SA 4.0',
      COPYRIGHT_LINK: 'https://creativecommons.org/licenses/by-sa/4.0/',
      COPYRIGHT_END: ' 共享, 转载注明出处.'
    },
    ERROR: {
      MESSAGE: '一些错误使我们在此相遇',
      TITLE: '页面无法找到',
      HELP_TEXT: '请尝试刷新页面, 或返回首页搜索, 若有疑问请联系我.',
      BACK_TO_HOME: '返回首页',
      CRAFTDOCS_ERROR: '当你看到这行字的时候, 说明你的配置页面格式有误, 请检查你的 Craft.do 配置文档.',
      LOADING: '正在加载....',
      TIMEOUT_TEXT: '加载时间太长? 点击直接访问 Notion 页面.'
    }
  },
  en: {
    NAV: {
      INDEX: 'Home',
      SEARCH: 'Search',
      ABOUT: 'About',
      FRINEDS: 'Friends',
      PROJECTS: 'Work',
      WRITING: 'Writing',
      NOTES: 'Notes',
      NEWSLETTER: 'Newsletter',
      BOOKS: 'Books',
      CONTACT: 'Contact'
    },
    PAGINATION: {
      PREV: 'Prev',
      NEXT: 'Next'
    },
    LAYOUT: {
      NOTICE_TEXT: 'If you have any questions, please contact me.',
      PAY_BUTTON: 'Sponsor',
      NOTICE_BUTTON: 'Contact',
      COMMENT_TITLE: 'Comments'
    },
    SEARCH: {
      ONLY_SEARCH: 'Only search tags',
      PLACEHOLDER: 'Search Posts',
      NOT_FOUND: 'No Posts Found'
    },
    HERO: {
      HOME: {
        CONTACT_BUTTON: 'Contact',
        CONTACT_BUTTON_DES: 'Collab / chat',
        RSS_BUTTON: 'Subscribe',
        PROJECTS_BUTTON: 'Projects',
        PROJECTS_BUTTON_DES: 'Open source & products',
        GREETING: "Hi, I'm"
      },
      NEWSLETTER: {
        SUBSCRIPTION_HEAD: 'Subscription Channels',
        TG_CHANNEL: 'Telegram Channel',
        RSS_BUTTON: 'Subscribe RSS'
      },
      NOTES: {
        TEXT_FOOTER: 'Automatically generated from the Craft.do page'
      },
      RSS_BUTTON_DES: 'RSS Reader',
      RSS_BUTTON_COPIED: 'Copied',
      RSS_BUTTON_DES_COPIED: 'Copied RSS URL'
    },
    HOME: {
      PROJECTS_TITLE: 'Featured work',
      PROJECTS_MORE: 'All projects',
      POSTS_TITLE: 'Latest writing',
      WORK_TITLE: 'Work',
      WORK_MORE: 'All work →',
      WRITING_TITLE: 'Writing',
      WRITING_MORE: 'All posts →',
      WRITING_SUB: 'Build logs and notes.'
    },
    PROJECTS: {
      TITLE: 'Work',
      DESCRIPTION: 'Products shipping, open-source proof, and labs — tiered, not flattened.',
      VISIT: 'Visit',
      FEATURED: 'Featured',
      ALL: 'All',
      TIER: {
        product: 'Product',
        oss: 'Open source',
        lab: 'Lab'
      },
      TIER_INTRO: {
        product: 'Products and tools shipping or live',
        oss: 'Identity and contributions on larger projects',
        lab: 'Experiments — may never become products'
      }
    },
    CONTACT: {
      TITLE: 'Contact',
      DESCRIPTION: 'For collabs, open source, or a quick hello — drop a message.',
      TG_DESCRIPTION: 'Or reach me on Telegram: ',
      SUCCESS_MESSAGE: 'Message sent, I will reply to you as soon as possible.',
      FORM_USERNAME: 'Your Name',
      FORM_EMAIL: 'Your Email*',
      FORM_CONTENT: 'Message',
      SEND_BUTTON: 'Send',
      FORM_EMAIL_REQUIRED: '*You can fill in other valid contact methods',
      FAILED_MESSAGE: 'Sorry, sending failed, please contact me directly on Telegram.'
    },
    FOOTER: {
      COPYRIGHT_START: 'This site is licensed under the ',
      COPYRIGHT_NAME: 'CC BY-SA 4.0',
      COPYRIGHT_LINK: 'https://creativecommons.org/licenses/by-sa/4.0/',
      COPYRIGHT_END: '.'
    },
    ERROR: {
      MESSAGE: 'Some errors have occurred',
      TITLE: 'Page Not Found',
      HELP_TEXT: 'Please try to refresh the page, or return to the home page and search, if you have any questions, contact me.',
      BACK_TO_HOME: 'Back to Home',
      CRAFTDOCS_ERROR: 'If you see this line, it means your Craft.do configuration page format is wrong.',
      LOADING: 'Loading....',
      TIMEOUT_TEXT: 'Waiting too long? Click to visit the Notion page directly.'
    }
  },
  ja: {},
  es: {}
}
